...
{
    length_of_header=0;    
    fread((void*)(&char_header), sizeof(char_header), 1, f);
...
void PlyParsing::ReadLine(pcl::PointCloud<PointType>::Ptr cloud,int linenum)
{
    if(POSITION==mode){
        Position ptmp;          
        int start_cursor=length_of_header+sizeof(ptmp)*linenum+1;
        fseek(f,start_cursor,0);
        fread((void*)(&ptmp), sizeof(ptmp), 1, f);      
...
void PlyParsing::LoadPly(string path)
{
    ...
    pcl::io::savePLYFileASCII("/home/llg/dataset_paper/out_of_core_version.ply",*cloud);
	...
	// 	float x, y, z;
	// 	fread((void*)(&x), 4, 1, f); x=x/1000;
	// 	fread((void*)(&y), 4, 1, f); y=y/1000;
	// 	fread((void*)(&z), 4, 1, f); z=z/1000;
	// 	float nx, ny, nz;
	// 	fread((void*)(&nx), 4, 1, f);
	// 	fread((void*)(&ny), 4, 1, f);
	// 	fread((void*)(&nz), 4, 1, f);
